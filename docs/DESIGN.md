...
**There is a shipped reference implementation.** Sentry's `@sentry/babel-plugin-component-annotate` parses JSX at build time and injects `data-sentry-component` and `data-sentry-source-file` onto the rendered DOM, for the same stated reason given here — CSS selectors are ambiguous, and worse after minification. That bounds the cost of this section and supplies a design to read. It also raises a question §4.2.1 does not ask: if a plugin stamps component names onto DOM attributes, the ancestry chain can be recovered by walking the **DOM** rather than the fiber tree, with no React internals involved at all. See [`PRIOR-ART.md`](PRIOR-ART.md) and the baseline comparison in [`VALIDATION-PLAN.md`](VALIDATION-PLAN.md) §3.2.

**❓ Unknown:** SWC (Next's default) vs Babel plugin authoring effort and maintenance burden across toolchains (Vite, CRA-legacy, Next, Remix). Non-trivial and toolchain-fragmented. *Partly answered — see the reference implementation above; what stays open is the SWC half and the Next/Remix/Vite matrix.*
...
**A later correction, from surveying the field ([`PRIOR-ART.md`](PRIOR-ART.md)):** the first half of that claim holds and the second is too generous. The behavioral-analysis layer is *not* differentiated on its own — path and journey analysis ship in PostHog, Amplitude and Mixpanel. What is differentiated is a flow graph **keyed on component identity rather than CSS selectors**, which is what would let it survive a refactor that breaks everyone else's. So identity is not merely the *existence condition* §19 calls it; identity is the differentiator, and the flow graph is how you demonstrate it.
...
4. **Build-plugin burden** — SWC vs Babel effort across Vite/Next/Remix/CRA? (§4.3) *Partly answered: Sentry ships and maintains a Babel version, so the cost is bounded and there is a reference implementation ([`PRIOR-ART.md`](PRIOR-ART.md)). The SWC half and the toolchain matrix stay open.*
